...
from src.modeling.modeling_hf_unimol import UniMolModel,UnimolConfig
import time
...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, t_to_sigma, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
        ...
        if self.confidence_mode:
            ...
            if not no_torsion:
                ...
                # self.tor_bond_conv_1x0o = TensorProductConvLayer(
                #     in_irreps=self.lig_conv_layers[-1].out_irreps,
                #     sh_irreps=self.final_tp_tor.irreps_out,
                #     out_irreps=f'1x0o',
                #     n_edge_features=3 * ns,
                ...
                # self.tor_bond_conv_1x0e = TensorProductConvLayer(
                #     in_irreps=self.lig_conv_layers[-1].out_irreps,
                #     sh_irreps=self.final_tp_tor.irreps_out,
                #     out_irreps=f'1x0e',
                #     n_edge_features=3 * ns,
    ...
    def forward(self, data):
        ...
        #res_edge_sigma = res_sigma[data['receptor'].batch][data['receptor', 'receptor'].edge_index[0]][data['receptor'].edge_mask]

        if self.scale_by_sigma:
            tor_pred = tor_pred * torch.sqrt(torch.tensor(torus.score_norm(edge_sigma .cpu().numpy())).float().to(tor_pred.device))
            #res_tor_pred = res_tor_pred * torch.sqrt(torch.tensor(torus.score_norm(res_edge_sigma .cpu().numpy())).float().to(tor_pred.device))
        
        # 预测配体的平移,旋转,扭转,氨基酸侧链的扭转
        return tr_pred, rot_pred, tor_pred #, res_tor_pred
