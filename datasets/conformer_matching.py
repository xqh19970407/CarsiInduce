...
def get_torsion_angles(mol):
    torsions_list = []
    G = nx.Graph()
    # 对分子内的所有原子进行循环,把编号加入图中
    for i, atom in enumerate(mol.GetAtoms()):
        G.add_node(i)
    nodes = set(G.nodes())# 保证不重复
    for bond in mol.GetBonds(): # 对分子中所有的键进行循环，然后把其添加到图中
        start, end = bond.GetBeginAtomIdx(), bond.GetEndAtomIdx() # 起始idx,结束idx
        G.add_edge(start, end)
    
    def connected_func(graph, num_sub_graphs=1):
        # 判断当前
        if num_sub_graphs==1:
            return nx.is_connected(graph)
        # print(len(list(nx.connected_components(graph))), num_sub_graphs)
        return len(list(nx.connected_components(graph))) == num_sub_graphs
    
    # 判断当前分子图是否是全连通的,返回全连通子图个数
    nsg = 1 if nx.is_connected(G) else len(list(nx.connected_components(G)))
    
    # 对所有的边进行循环，然后进行判断该键是否是可旋转键然后添加到图中
    for e in G.edges():
        G2 = copy.deepcopy(G)
        G2.remove_edge(*e) # 删除当前边
        if connected_func(G2, nsg): continue # 删除当前键之后如果还是全连通或者连通图的个数没有增加那么该键就不是可旋转键,直接跳过(排除键在环上的情况)
        l = list(sorted(nx.connected_components(G2), key=len)[0]) # 对所有连通图的原子数进行排序,取出原子数少的图,返回原子列表
        if len(l) < 2: continue # 如果该图是一个原子的话(排除单原子的情况),说明当前键不是可旋转键
        n0 = list(G2.neighbors(e[0])) # 从e[0]原子得到周围所有原子
        n1 = list(G2.neighbors(e[1])) # 从e[1]原子得到周围所有原子
        torsions_list.append(
            (n0[0], e[0], e[1], n1[0])
        )
        # print(torsions_list[-1])
