...
def get_calpha_graph(rec, c_alpha_coords, n_coords, c_coords, complex_graph, cutoff=20, max_neighbor=None, lm_embeddings=None):
    ...
    distances = spa.distance.cdist(c_alpha_coords, c_alpha_coords) #氨基酸alpha C的距离矩阵
    src_list = []
    dst_list = []
    mean_norm_list = []
    for i in range(num_residues):
        ...
        diff_vecs = c_alpha_coords[src, :] - c_alpha_coords[dst, :]  # (neigh_num, 3) 计算出24个近邻氨基酸和alpha_C之间的向量
        mean_vec = weights.dot(diff_vecs)  # (sigma_num,neig_num)  @  (neigh_num, 3)  >>> (sigma_num, 3)    [5,3]  对距离向量做加权
        # 计算alpha_c和近邻氨基酸的距离  (sigma_num,neig_num) @ (neigh_num,) >>>  (sigma_num,)   [5,]
        # [5,24] @ [24,] >>> [5]
    ...
    # [N_res,5]   做一个循环，以每个氨基酸为中心，计算距离他15A以内的最多24个氨基酸，按照5种粒度，得到距离的5类权重，再和距离做乘法，最终得到5类特征
    # 对于每个氨基酸都得到5个粒度的特征
