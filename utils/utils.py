...
from models.score_model import FlexibleDocking_resTR_atomT
...
def get_model(args, device, no_parallel=False, confidence_mode=False, inference=False):
    
    model_class = FlexibleDocking_resTR_atomT #EncoderDecoder2 #  #  #FlexibleDocking_resTR_atomT #FlexibleDockingModel AttentionEncoderDecoder   #
    lm_embedding_type = None
    if args.esm_embeddings_path is not None: lm_embedding_type = 'esm'
    model = model_class(inference=inference,
...
def get_uni_score_model(args, t_to_sigma, no_parallel=False, confidence_mode=False):
    from models.unimol_score import UniScoreModel
    model_class = UniScoreModel
    ...
    score_config ={"t_to_sigma":t_to_sigma,
            'no_torsion':args.no_torsion,
            "timestep_emb_func":timestep_emb_func,
            'num_conv_layers':args.num_conv_layers,
            'lig_max_radius':args.max_radius,
            'rec_max_radius':args.receptor_radius,
            'scale_by_sigma':args.scale_by_sigma,
            'sigma_embed_dim':args.sigma_embed_dim,
            'ns':args.ns, 
            'nv':args.nv,
    ...
    model = model_class(scoreconfig=score_config)
    return model
    ...
    def update(self, parameters):
        ...
        with torch.no_grad():
            ...
            for s_param, param in zip(self.shadow_params, parameters):
                s_param.to(param.device).sub_(one_minus_decay * (s_param.to(param.device) - param))
