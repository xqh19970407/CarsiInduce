...
def sample(eps):
    eps_idx = (np.log10(eps) - np.log10(MIN_EPS)) / (np.log10(MAX_EPS) - np.log10(MIN_EPS)) * N_EPS # 取对数标准化等
    eps_idx = np.clip(np.around(eps_idx).astype(int), a_min=0, a_max=N_EPS - 1) # 上下界的截断函数取整数

    x = np.random.rand() # 随机产生数字
    return np.interp(x, _cdf_vals[eps_idx], _omegas_array) # 插值函数返回随机数的插值
...
def res_sample_vec(res_shape, eps):
    (res_num, dim) = res_shape
    x = np.random.randn(res_num, dim)
...
def res_score_vec(eps, vec):
    eps_idx = (np.log10(eps) - np.log10(MIN_EPS)) / (np.log10(MAX_EPS) - np.log10(MIN_EPS)) * N_EPS
    eps_idx = np.clip(np.around(eps_idx).astype(int), a_min=0, a_max=N_EPS - 1)

    om = np.linalg.norm(vec, axis=1, keepdims=True)# [num_res]
    return np.interp(om, _omegas_array, _score_norms[eps_idx]) * vec / om
